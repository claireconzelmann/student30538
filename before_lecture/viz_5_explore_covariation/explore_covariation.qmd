...
# Roadmap 
Explore penguin `color` (continuous) vs. `cut` (categorical)
* boxplot by category
* densities by category
(no summary at the end of this section)
...
## continuous & categorical: `transform_density()` filled in
...
# Roadmap 
* Exploring `diamonds` (`color` v `price`) with a table
* Plots: weighting by size vs. by color

## Question: In `diamonds`, how is cut related to color? 2 categorical vars
```{python}
#| warning: false
...
Discussion question: can you describe what the distribution of diamonds across different cuts looks like? What about across different colors?
...
Let's consider 3 ways to represent this relationship. 

Discussion: for each one, what is the
